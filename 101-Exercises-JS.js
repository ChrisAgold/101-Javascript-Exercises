...
//  return x.replace(/[aeiou]/ig,'');
// }
function removeVowels(str) {
  var vowels = "aeiou";
  var strArray = str.split(""); // splits string and leaves no spaces between each letter //
  var arr = [];
  for (var i = 0; i < strArray.length; i++) {
    if (vowels.indexOf(strArray[i]) < 0) { // indexOf counting the array and returns a negative number for vowels //
      arr.push(strArray[i]);
    }
...
function endsWithVowel(word) {
  var vowels = ("aeiouAEIOU");
  return vowels.includes(word.slice(-1))
...
function startsAndEndsWithVowel(word) {
  var vowels = ("aeiouAEIOU");
  return vowels.includes(word[0] || word.slice(-1))
}
...
// function mean(arr) {
//   var total = (a, b) => a + b;
//   return arr.reduce(total) / arr.length;
// }
...
function lowestPriceBook(input) {
  const low =books.reduce(function(min, book) {
...
function mostSpentOnItem(obj){
  ...
  for(var i=0;i<obj.items.length;i++) {
    if((obj.items[i].price * obj.items[i].quantity) > (highest.price * highest.quantity)) {
      highest = obj.items[i];
    }
  }
  return highest;
